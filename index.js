...
*/
function getName(Objectreturn) {
  return 'Hello, my name is ' + Objectreturn.name
  }
  
  getName(Objectreturn = {
    id: 1,
    name: 'Leia',
    email: 'leia@leia.com'
  });
...
*/
function getOlderCars(inventory, a) {
  let newArr = [];
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_year <= a) {
...
function getGermanCars(inventory) {
  ...
  for (i = 0; i < inventory.length; i++){
    ...
    inventory[i].car_make === 'BMW' || inventory[i].car_make === 'Mercedes-Benz' || inventory[i].car_make === 'Volkswagen'){
